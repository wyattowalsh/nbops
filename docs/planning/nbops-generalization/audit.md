...
| G3 / TASK-003 | Inspect stats/outline/imports/outputs | `list_outputs`; CLI/API `outputs`; inspect payload includes outputs; `extract_imports` is IPython-aware and skips non-Python kernels and file-body magics; outline/split/NB002 share ATX+setext heading extraction that skips fences | verified (recovered) |
